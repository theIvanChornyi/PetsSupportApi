...
const { userNameRegExp, commentRegExp } = require('../../helpers/regExpressions');
...
const schemaNotice= Joi.object({
  ...
  category: Joi.string().required().valid("lostFound", "inGoodHands", "sell").default('sell'),
...
const addNotice = async (req, res) => {
  ...
  const notice = await Notice.create({
    ...
    // await User.findByIdAndUpdate(owner, { $push: { notice: data._id } });
